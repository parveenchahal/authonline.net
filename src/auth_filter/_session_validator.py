...
def init_session_validator(logger: Logger, jwt_handler: JWTHandler, session_handler: SessionHandler):
    global _session_validator
    if _session_validator is None:
        _session_validator = _SessionValidator(logger, jwt_handler, session_handler)
    else:
